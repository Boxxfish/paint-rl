...
ref_imgs = []
for dir in ds_path.iterdir():
    ref_img = Image.open(dir / "outlines.png")
    ref_img = ref_img.resize((img_size, img_size))
    ref_imgs.append(np.array(ref_img).transpose([2, 0, 1])[1] / 255.0)

env = gym.vector.SyncVectorEnv(
    [
        lambda: NormalizeReward(TimeLimit(StrokeEnv(img_size, ref_imgs=ref_imgs), 10))
        for _ in range(num_envs)
    ]
)
test_env = StrokeEnv(img_size, ref_imgs=ref_imgs[:20], render_mode="human")
del ref_imgs
