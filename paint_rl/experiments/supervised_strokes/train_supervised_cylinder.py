        ...
        try:
            ...
            for circle in circles:
                ...
                if a < b:
                    ...
                    a = b
                    b = temp
                rot = float(circle["rot"]) / orig_img_size

                p1 = (center - np.array([cos(rot), sin(rot)]) * a)
                p2 = (center + np.array([-sin(rot), cos(rot)]) * b * 2)
                p3 = (center + np.array([cos(rot), sin(rot)]) * a)
                p4 = (center - np.array([-sin(rot), cos(rot)]) * b * 2)
            ...
            a_1 = max(int(circle_1["a"]) / orig_img_size, int(circle_1["b"]) / orig_img_size)
            rot_1 = float(circle_1["rot"]) / orig_img_size
            
            circle_2 = circles[first_c_index - 1]
            center_2 = np.array(circle_2["center"]) / orig_img_size
            a_2 = max(int(circle_2["a"]) / orig_img_size, int(circle_2["b"]) / orig_img_size)
            rot_2 = float(circle_2["rot"]) / orig_img_size

            c1_r = (center_1 + np.array([cos(rot_1), sin(rot_1)]) * a_1)
            c1_l = (center_1 - np.array([cos(rot_1), sin(rot_1)]) * a_1)
            c2_r = (center_2 + np.array([cos(rot_2), sin(rot_2)]) * a_2)
            c2_l = (center_2 - np.array([cos(rot_2), sin(rot_2)]) * a_2)
